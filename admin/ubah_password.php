                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            if (isset($_POST["ubah"])) {
                                ...
                                $id_admin = $_SESSION["admin"]["id_admin"]; //Ambil Id_Petugas Di Dalam Session.
                                $ambil = $konek->query("SELECT * FROM admin WHERE id_admin='$id_admin' AND password=md5('$password_lama')");
                                $cek_pass = $ambil->num_rows;

                                //Jika Tidak ada Pass yang sesuai 
                                if (!$cek_pass >= 1) {
                                    echo "<script>alert('Maaf, Password Lama Anda Tidak Sesuai');</script>";
                                    echo "<script>location='index?halaman=ubah_password'</script>";
                                }
                                ...
                                //Selain Itu. Atau jika password baru dan konfirmasi password sama. 
                                    ...
                                    $konek->query("UPDATE admin SET password=md5('$password_baru') WHERE id_admin='$id_admin'");
                                    ...
                                    echo "<script>location='index?halaman=logout'</script>"; //Redirect(Melakukan) Kembali ke Halaman Login.
                                    session_destroy();
                                }
                            }
                            ?>
