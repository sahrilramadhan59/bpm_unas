...
if (isset($_POST["hapus"])) {
	...
	for ($i = 0; $i < sizeof($kode_standart); $i++) {
		...
		WHERE tbl_standart.id_user = '$id_user' AND tbl_standart.id_standart='$kode_standart[$i]'"); //Ini untuk mengambil data 
		...
		$Cek = $konek->query("DELETE FROM tbl_standart WHERE id_standart='$kode_standart[$i]' AND id_user='$id_user'");
	}
